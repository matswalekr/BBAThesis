...
def download_raw_data(
    ...
    factors_monthly_raw: pd.DataFrame = pd.read_csv(config.paths.raw_read(FILENAMES.FF5_factors_monthly),
                                                parse_dates=['date'], index_col='date')
    factors_yearly_raw: pd.DataFrame = pd.read_csv(config.paths.raw_read(FILENAMES.FF5_factors_yearly),
                                                parse_dates=['date'], index_col='date')

    stock_prices_raw: pd.DataFrame = pd.read_csv(config.paths.raw_read(FILENAMES.Stock_prices),
                                                parse_dates=['date'], index_col='date')
    
    firm_info_raw: pd.DataFrame =   pd.read_csv(config.paths.raw_read(FILENAMES.Firm_info))

    sic_desc_raw: pd.DataFrame =   pd.read_csv(config.paths.raw_read(FILENAMES.Sic_description))

    return factors_monthly_raw, factors_yearly_raw, stock_prices_raw, firm_info_raw, sic_desc_raw
...
def remove_firms_missing_sharesoutstanding(
    ...
    mask_activity = (
        stock_price
        .groupby("gvkey")["sharesoutstanding"]
        .apply(lambda s: s.le(0).sum() / s.size < threshold_missing_shares)
    )
...
def clean_stock_prices(
    ...
    stock_prices_raw[numeric_cols] = stock_prices_raw[numeric_cols].apply(pd.to_numeric, errors="coerce")

    # Remove firms with missing shares outstanding
    stock_prices_cleaned = remove_firms_missing_sharesoutstanding(stock_prices_raw, config)
...
def fill_missing_values(
    ...
    stock_price["sharesoutstanding"] = stock_price.groupby("gvkey")["sharesoutstanding"].ffill()

    # Fill the nas in close with the mean of previous and last
    stock_price["close"] = (
        stock_price.groupby("gvkey")["close"]
...
def intersect_stockprices_monthlyfactors(
    ...
    full_idx: pd.datetimeIndex = pd.date_range(stock_prices_cleaned["date"].min(), factors_monthly_processed.index.max(), freq="D")
    ...
    stock_prices_reindexed = (
        ...
        .apply(lambda g: g.reindex(pd.MultiIndex.from_product(
            [[g.index.get_level_values("gvkey")[0]], full_idx],
            names=["gvkey", "date"]
        )).ffill(limit=2)).reset_index(level="gvkey")
    )

    # Intersect the dates with the Fama French factors
    common_dates = stock_prices_reindexed.index.intersection(factors_monthly_processed.index)


    # Only keep the common dates
    stock_prices_common_date: pd.DataFrame = stock_prices_reindexed.loc[common_dates].sort_index()
...
def save_processed_data(
    ...
    factors_monthly_processed.to_csv(config.paths.processed_out(FILENAMES.FF5_factors_monthly))
    factors_yearly_processed.to_csv(config.paths.processed_out(FILENAMES.FF5_factors_yearly))

    stock_prices_intersected.to_csv(config.paths.processed_out(FILENAMES.Stock_prices))

    firm_info_processed.to_csv(config.paths.processed_out(FILENAMES.Firm_info),
                        index=False)
    
    sic_desc_processed.to_csv(config.paths.processed_out(FILENAMES.Sic_description), 
                    index=False)
...
def clean_data(
    ...
    # Download the data
    factors_monthly_raw, factors_yearly_raw, stock_prices_raw, firm_info_raw, sic_desc_raw = download_raw_data(config)

    # Process the factor data
    factors_monthly_processed, factors_yearly_processed = clean_factors(factors_monthly_raw, factors_yearly_raw, config)
    ...
    stock_prices_intersected: pd.DataFrame = intersect_stockprices_monthlyfactors(stock_prices_cleaned, factors_monthly_processed, config)
    ...
    return factors_monthly_processed, factors_yearly_processed, stock_prices_intersected, firm_info_processed, sic_desc_processed
...
def clean_save_data(
    ...
    factors_monthly_processed, factors_yearly_processed, stock_prices_intersected, firm_info_processed, sic_desc_processed = clean_data(config)
