...
type EntireData = Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]
...
def download_prices_daily_wrds(
    ...
    price_query_params: List[str, str] = (start_date.strftime("%Y-%m-%d"),end_date.strftime("%Y-%m-%d"))
    
    # Get the SQL query
    sql_query_daily_price: str = config.paths.sql_query("daily_market_prices").read_text()
    
    return con.raw_sql(sql_query_daily_price, 
                       params=price_query_params, 
                       date_cols=['date'])
...
def save_data(
    ...
    ff5_monthly.to_csv(
        CONFIG.paths.raw_out(FILENAMES.FF5_factors_monthly))
    ff5_yearly.to_csv(
        CONFIG.paths.raw_out(FILENAMES.FF5_factors_yearly))
    
    prices_obs_universe.to_csv(
        CONFIG.paths.raw_out(FILENAMES.Stock_prices),
        index=False)

    firm_info.to_csv(
        CONFIG.paths.raw_out(FILENAMES.Firm_info),
        index=False)

    sic_codes.to_csv(
        CONFIG.paths.raw_out(FILENAMES.Sic_description),
        index=False)
    
    return

def download_save_raw_data(
    config : CONFIGURATION
)->None:
    """
