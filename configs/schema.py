...
class BASIC_CONFIG_CLASS():
    ...
    def get_latest(self, 
                   ...
                   type_: ALLOWED_TYPE,
                   suffix: str, 
                   date_pattern: str = r"\d{4}-\d{2}-\d{2}")->Path:
        """
        ...
        rx = re.compile(rf"^{re.escape(stem)}_(?P<date>{date_pattern}){re.escape(suffix)}$")
        matches = []
        ...
        if not matches:
            raise FileNotFoundError(f"No files found for pattern {stem}_<date>{suffix} in {directory}")
    ...
    def create_filename_with_date(self,
                                stem: str, 
                                type_ : str,
                                suffix: str,
                                date_pattern: str = "%Y-%m-%d")->Path:
        """ 
...
class CONFIGURATION():
    ...
    def __post_init__(self):
        ...
        if (self.END_DATE_FACTORS_DOWNLOAD < self.START_DATE_FACTORS_DOWNLOAD):
            raise ValueError("END_DATE_FACTORS_DOWNLOAD must be after START_DATE_FACTORS_DOWNLOAD")
        ...
        if (self.PORTFOLIO_AGGREGATION_METHOD not in {"MarketCap", "Equal"}):
            raise ValueError("PORTFOLIO_AGGREGATION_METHOD must be 'MarketCap' or 'Equal'")
        
        if not (self.BREAK_DATE_PERIODS is None) ^ (self.PERIOD_WINDOW_LENGTH_MONTHS is None):
            raise ValueError("Either BREAK_DATE_PERIODS or PERIOD_WINDOW_LENGTH_MONTHS must be provided, but not both")

        if not (self.MARKETCAP_PORTFOLIO_NUMBER_FIRMS is None) ^ (self.MARKETCAP_PORTFOLIO_PERCENTILE is None):
            raise ValueError("Either MARKETCAP_PORTFOLIO_NUMBER_FIRMS or MARKETCAP_PORTFOLIO_PERCENTILE can be provided, but not both")

        if (self.T_TEST_SIGNIFICANCE_LEVEL <= 0):
            raise ValueError("T_TEST_SIGNIFICANCE_LEVEL must be positive")
        
        if (self.THRESHOLD_MISSING_SHARESOUTSTANDING < 0 or self.THRESHOLD_MISSING_SHARESOUTSTANDING > 1):
            raise ValueError("THRESHOLD_MISSING_SHARESOUTSTANDING must be between 0 and 1")
